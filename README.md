...
- 🎯 Entry points to execute your program using `python -m <lp_endpoints>` or `$ lp_endpoints` with basic CLI argument parsing.
...
---
# lp_endpoints Flask Application

Awesome lp_endpoints created by knopperman
...
```bash
git clone https://github.com/knopperman/lp-endpoints lp_endpoints
cd lp_endpoints
make install
```
...
```bash
lp_endpoints create-db   # run once
lp_endpoints populate-db  # run once (optional)
lp_endpoints add-user -u admin -p 1234  # ads a user
lp_endpoints run
```
