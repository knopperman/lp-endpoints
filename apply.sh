...
original_name="lp_endpoints"
original_urlname="lp-endpoints"
original_description="Awesome lp_endpoints created by knopperman" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/lp_endpoints "${name}"
...
then
    ...
    echo "then run 'lp_endpoints' or 'python -m lp_endpoints'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'lp_endpoints' or 'python -m lp_endpoints' or 'poetry run lp_endpoints'"
fi
